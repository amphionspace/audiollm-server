...
ASR 送模前会执行服务端人声门控与过滤。低人声证据音频可能不调用 ASR 模型并返回空 `asr.text`；已放行音频可能在送模前裁去非人声区间。该行为由 `asr_segment_voice_gate_*` / `asr_segment_voice_filter_*` 服务端配置控制，请求不能临时覆写。
...
- [通用 ASR 上传](../protocols/transcribe-streaming-protocol.md#rest-上传接口)
- [整段情感识别 HTTP](../protocols/emotion-streaming-protocol.md)
