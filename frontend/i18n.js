...
(() => {
  ...
  const EN = {
    ...
    'enhanced.greeting.html': 'Add your API Key, choose the enhancements you need, then start speaking.<br/><br/><span class="text-muted">Raw recognition appears first; the refined result follows as it is generated.</span>',
    'enhanced.settings.title': 'Recognition enhancements',
    'enhanced.settings.tip': 'Settings apply to the next recording.',
    'enhanced.key.label': 'API Key',
    'enhanced.key.tip': 'Kept only in this page and sent directly to amphion.top.',
    ...
    'enhanced.error.keyRequired': 'Enter an API Key first',
    'enhanced.error.auth': 'API Key authentication failed.',
  ...
  const ZH = {
    ...
    'enhanced.greeting.html': '填写 API Key，选择需要的增强能力，然后开始说话。<br/><br/><span class="text-muted">原始识别会先实时出现，精修结果随后逐步生成。</span>',
    'enhanced.settings.title': '识别增强',
    'enhanced.settings.tip': '以下设置将在下一次录音时生效。',
    'enhanced.key.label': 'API Key',
    'enhanced.key.tip': '仅保留在当前页面，并直接发送到 amphion.top。',
    ...
    'enhanced.error.keyRequired': '请先填写 API Key',
    'enhanced.error.auth': 'API Key 鉴权失败。',
